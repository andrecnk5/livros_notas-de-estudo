# Lógica de Programação e Algoritmos com JavaScript

## Sumário

### [1. Lógica](capitulo-01/README.md)

- [Lógica de programação ](capitulo-01/cap1-01.md)
- [Entrada, processamento e saída ](capitulo-01/cap1-02.md)
- [JavaScript ](capitulo-01/cap1-03.md)
- [Editores de código JavaScript ](capitulo-01/cap1-04.md)
- [Saída de dados com `alert()` e `console.log()` ](capitulo-01/cap1-05.md)
- [Variáveis e constantes ](capitulo-01/cap1-06.md)
- [Entrada de dados com `prompt()` ](capitulo-01/cap1-07.md)
- [Comentários ](capitulo-01/cap1-08.md)
- [Exemplos de entrada, processamento e saída ](capitulo-01/cap1-09.md)
- [Exercícios ](capitulo-01/cap1-10.md)

### [2. Operadores em JavaScript](capitulo-02/README.md)

- [Operadores Aritméticos](capitulo-02/cap2-01-operadores_aritmeticos_javascript)
- [Operadores de Atribuição](capitulo-02/cap2-02-operadores_de_atribuicao_javascript)
- [Operadores de Comparação](capitulo-02/cap2-03-operadores_de_comparacao_javascript)
- [Operadores Lógicos](capitulo-02/cap2-04-operadores_logicos_javascript)

### [3. Integração com HTML](capitulo-03/README.md)

- [3.1 Estrutura básica de um documento HTML](capitulo-03/cap3-01.md)
- [3.2 Cabeçalhos, parágrafos e campos de formulário](capitulo-03/cap3-02.md)
- [3.3 Criação do programa JavaScript](capitulo-03/cap3-03.md)
- [3.4 Métodos `querySelector()` e `getElementById()`](capitulo-03/cap3-04.md)
- [3.5 Introdução a eventos e funções](capitulo-03/cap3-05.md)
- [3.6 Propriedades `innerText`, `innerHTML` e `value`](capitulo-03/cap3-06.md)
- [3.7 Método `document.write()`](capitulo-03/cap3-07.md)
- [3.8 Método `preventDefault()`](capitulo-03/cap3-08.md)
- [3.9 Operadores aritméticos e funções matemáticas](capitulo-03/cap3-09.md)
- [3.10 Operadores relacionais](capitulo-03/cap3-10.md)
- [3.11 Exemplos de programas JavaScript integrados com HTML](capitulo-03/cap3-11.md)
- [3.12 Exercícios](capitulo-03/cap3-12.md)
- [3.13 Considerações finais do capítulo](capitulo-03/cap3-13.md)

---

## Referências

## Índice remissivo
